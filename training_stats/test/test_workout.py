...
def test_workout_with_exercise():
    workout = Workout(date=date(2017, 1, 1))
    workout.addExercise(name='Knäböj', reps=8, weight=100)
    workout.addExercise(name='Knäböj', reps=8, weight=101)
...
def test_workout_total_weight_sums_weight():
    workout = Workout(date=date(2017, 1, 1))
    workout.addExercise(name='a', reps=1, weight=1)
...
def test_workout_total_weight_sums_weight_and_reps():
    workout = Workout(date=date(2017, 1, 1))
    workout.addExercise(name='a', reps=2, weight=1)
    assert workout.total_weight() == 2
    workout.addExercise(name='a', reps=1, weight=1)
...
def test_workouts_total_weight_by_date_series():
    ...
    w2.addExercise(name='a', reps=2, weight=1)
    workouts.addWorkout(w2)

    w1 = Workout(date=date(2017, 1, 1))
    w1.addExercise(name='a', reps=1, weight=1)
    workouts.addWorkout(w1)
